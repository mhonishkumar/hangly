...
class RopeSimulation {
  ...
  beginAnchorDrag() { this.isDraggingAnchor = true; }
  updateAnchorDrag(dx, dy) {
    this.anchor.x += dx;
    this.anchor.y += dy;
  }
  endAnchorDrag() { this.isDraggingAnchor = false; }
...
function loadCharm(pathOrUrl) {
  ...
  // URL-encode file paths so special chars (spaces, ğ, é, etc.) work on web servers
  if (!pathOrUrl.startsWith('data:') && !pathOrUrl.startsWith('http')) {
    const parts = pathOrUrl.split('/');
    parts[parts.length - 1] = encodeURIComponent(parts[parts.length - 1]);
    pathOrUrl = parts.join('/');
  }
...
pickerContainer.addEventListener('mouseenter', () => isHoveringUI = true);
pickerContainer.addEventListener('mouseleave', () => isHoveringUI = false);

// Anchor drag zone
function isInAnchorZone(x, y) {
  const ax = sim.anchor.x;
  const ay = sim.anchor.y;
  return Math.abs(x - ax) < 80 && Math.abs(y - ay) < 30;
}
...
function drawAnchor(nodes) {
  ...
  ctx.fillStyle = darkMode ? 'rgba(255,255,255,0.6)' : 'rgba(80,60,40,0.8)';
  ctx.fill();
}
...
function animate(currentTime) {
  ...
  // Check for sleep
  if (!sim.isDragging && !sim.isDraggingAnchor && sim.isSettled()) {
    sleepFrames++;
...
// ─── Init ─────────────────────────────────────────────────────────────────────
resizeCanvas();
sim.anchor.x = window.innerWidth / 2;
sim.anchor.y = 50;
sim.reset();
lastTime  = performance.now();
rafHandle = requestAnimationFrame(animate);
